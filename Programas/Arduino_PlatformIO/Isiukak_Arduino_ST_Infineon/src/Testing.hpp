...
uint16_t Testing_tiempo_VolanteDetenido = 2000;
...
uint16_t Testing_volanteDetenido_nIteraciones;
...
uint8_t Testing_VolanteDetenido_Probado = false;
...
//*** Messages ***//
                                             // "1       10        20        30"
char Testing_AceleradorActivado_msg[40] =       "1. Gas pedal full activated\0";
char Testing_FrenoActivado_msg[40] =            "2. Brake pedal full activated\0";
char Testing_VolanteIzqActivado_msg[40] =       "3. Steering wheel full left\0";
char Testing_VolanteDetenido_msg[40] =          "4. Steering wheel stoped\0";
...
void calculate_IterationsPerActuator(uint8_t periodoDeseado)
{
    ...
    Testing_volanteDetenido_nIteraciones    = Testing_tiempo_VolanteDetenido / periodoDeseado;
...
void EmbeddedTest_SetInputs_and_Status()
{
    ...
    **************************************************************************************/
    ...
    {
        if(contTesting == 1) {
            Serial.println("**I. Testing the system at normal behaviour (everything connected and in range)**");
    ...
    {
        /*if(contTesting == 1) {
            Serial.println("**Testing Freno a full...**"); // Done in Testing_ActualizarContadorMsg()
        }*/
    ...
    else if(contTesting <= Testing_volanteDetenido_nIteraciones
                       && !Testing_VolanteDetenido_Probado)
    {
        if(contTesting == 1)
        {
            Serial.println("**4. Testing Volante Stopped...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_Center, joyX_Center);
        // Testing_VerificacionSeguridad_SetValues //No modificar las variables de conexión

        Testing_ActualizarContador(contTesting, Testing_volanteDetenido_nIteraciones,
                                   Testing_VolanteDetenido_Probado);
    }
                       ...
                       && !Testing_VolanteDerecha_Probado)
    {
        if(contTesting == 1)
        {
            Serial.println("**5. Testing Volante a la derecha...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_Center, joyX_MaxVal);
        // Testing_VerificacionSeguridad_SetValues //No modificar las variables de conexión.

        Testing_ActualizarContador(contTesting, Testing_volanteDerecha_nIteraciones,
                                   Testing_VolanteDerecha_Probado);
    }
    ...
    {
        if(contTesting == 1) {
            Serial.println("**II. System at error: Disconnected components**");
            Serial.println("**6. Joystick Y Disconnected...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_MaxVal, joyX_Center);
    ...
    {
        if(contTesting == 1)  {
            Serial.println("**7. Joystick X Disconnected...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_MaxVal, joyX_Center);
    ...
    {
        if(contTesting == 1) {
            Serial.println("**8. Joystick both axis Disconnected...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_MaxVal, joyX_Center);
    ...
    {
        if(contTesting == 1) {
            Serial.println("**9. Freno Desconectado...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_MaxVal, joyX_Center);
                       ...
                       && !Testing_ActuadorAcelerador_Desconectado_Probado)
    {
        if(contTesting == 1)
        {
            Serial.println("**10. Acelerador Desconectado...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_MaxVal, joyX_Center);
    ...
    {
        if(contTesting == 1) {
            Serial.println("**11. Ambos Actuadores Desconectados...**");
        }
    ...
    {
        if(contTesting == 1) {
            Serial.println("**III. System at error: Out of Range components**");
            Serial.println("**12. Joystick Y Out of Range...**");
        }
        Testing_set_Virtual_Joystick_Pots(joyY_MaxVal, joyX_Center);
    ...
    // 13  Testing Joystick X Out of Range.
    ...
    // 14  Testing Joystick both axis Out of Range.
    ...
    // 16  Testing Brake pedal Out of Range.
    ...
    // 17  Testing Both Actuators Out of Range.
                       ...
                       && !Testing_ReiniciarPrueba_Probado)
    {
        ...
        if(contTesting == Testing_ReiniciandoPrueba_nIteraciones)
        {
            ...
            Testing_VolanteDetenido_Probado     = false;
