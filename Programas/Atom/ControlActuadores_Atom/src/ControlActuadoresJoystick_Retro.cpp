...
//int pinLedPosibleFalsoCorriente = 6;
//int pinMapeoCompletoVolante = 6;
...
void setup()
{
  ...
  //pinMode(pinMapeoCompletoVolante, INPUT);
  //pinMode(pinLedEncoderDesconectado, OUTPUT);
  //pinMode(pinLedPosibleFalsoCorriente, OUTPUT);
  //pinMode(pinLedJoystickConFalso, OUTPUT);
  //pinMode(pinLedMapeoCompletoVolante, OUTPUT);
  //pinMode(pinLedActuadoresDesconectados, OUTPUT);
  ...
  //Serial.println("Despues LeerEncoder");
  ...
  if(NoParoManualEmergencia)
  {
    if(EncoderConectado && potenciometrosConectados)
    {
      ...
      if( (tiempoActual - tiempoAnterior) >= periodo )
      {
        ...
        contadorIteraciones++;
        if(contadorIteraciones >= 25) //Aprox. cada 500 ms.
        {
          potenciometrosConectados = VerificarConexionPotenciometros(cantidadPots);
    ...
    {
      ParoDeEmergencia(/*pinLedMapeoCompletoVolante*/); //Aquí se siguen leyendo los sensores, en caso de que se reconecten.
